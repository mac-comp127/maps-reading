...
- You know how to use Java's Map interface: you know how to create a map, add information to it, and retrieve information from it.
...
A [Map](https://docs.oracle.com/en/java/javase/21/docs/api/java.base/java/util/Map.html) is, in addition to List and Set, one of the most widely used of Java's pre-built data structures. A map is used whenever data is stored as key-value pairs, where values can be added, retrieved, and deleted using keys. (Other languages also call this data structure a “dictionary,” “associative array,” or “hash table.”)
...
In the example below, a HashMap object has been created to search for cities by their postal codes, after which four postal code-city pairs have been added to the map. At the end, the postal code "00710" is retrieved from the map. Both the postal code and the city are represented as strings.


<!-- ```java
Map<String, String> postinumerot = new HashMap<>();
...
Hajautustaulua luodessa tarvitaan kaksi tyyppiparametria, avainmuuttujan tyyppi ja lisättävän arvon tyyppi. Mikäli hajautustaulussa on avaimena merkkijono (String) ja arvona luku (Integer) luodaan hajautustaulu komennolla `Map<String, Integer> hashmap = new HashMap<>();`
...
<blockquote class="programming-exercise" name='Nicknames' tmcname='part08-Part08_06.Nicknames'>
...
Let's create a map that uses the book's name as a key, i.e., a String-type object, and the book we've just created as the value.

<!-- ```java
Map<String, Book> hakemisto = new HashMap<>();
``` -->
```java
...
The map above uses a`String` object as a key. Let's expand the example so that two books are added to the directory, `"Sense and Sensibility"` and `"Pride and Prejudice"`.
...
Books can be retrieved from the directory by book name. A search for `"Persuasion"` will not produce any results, in which case the map returns a `null`-reference. The book "Pride and Prejudice" is found, however.
...
<blockquote class="programming-exercise" name='Abbreviations' tmcname='part08-Part08_07.Abbreviations'>
...
<blockquote class="programming-exercise" name='Print me my hash map' tmcname='part08-Part08_08.PrintMeMyHashmap'>
...
<blockquote class="programming-exercise" name='Print me another map' tmcname='

*Programming exercise*part08-Part08_09.PrintMeAnotherHashmap':>
...
<!-- ```java
Map<Integer, String> hashmap = new HashMap<>(); // toimii
hashmap.put(1, "Ole!");
Map<int, String> taulu2 = new HashMap<>(); // ei toimi
``` -->
```java
...
<blockquote class="programming-exercise" name='I owe you' tmcname='part08-Part08_10.IOweYou'>
...
_This course material is licensed under a [Creative Commons BY-NC-SA 4.0](https://creativecommons.org/licenses/by-nc-sa/4.0/deed) license._
